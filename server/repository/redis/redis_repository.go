...
func NewRedisRepository(redisClient *redis.Client) interfaces.RedisRepository {
...
func (r *RedisRepository) StoreAssetCache(assetID string, assetType string, data *pkg.AssetCache, expiry time.Duration) error {
...
func (r *RedisRepository) GetAssetCache(assetID string, assetType string) (*pkg.AssetCache, error) {
	key := fmt.Sprintf("asset:%s:%s", assetType, assetID)
