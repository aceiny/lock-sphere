...
export function useCreateOrCheckMasterKey() {
  return useMutation({
    mutationFn: async (master_key: string) => {
      const { data } = await axios.post<User>("/user/master-key", {
        master_key,
      });
      return data;
    },
...
export function useUpdateProfilePicture() {
  ...
  return useMutation<
    User,
    Error,
    { formData: FormData; options?: UploadOptions }
  >({
